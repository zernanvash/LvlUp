<div class="space-y-6">
    <div class="lvl-panel p-8 border-l-4" style="border-left-color: var(--lvl-p400) !important;">
        <h2 class="font-display text-xl font-bold text-white mb-1 flex items-center gap-2">
            <i class="fas fa-eye text-purple-400"></i> Profile Visibility
        </h2>
        <p class="text-purple-300/70 text-sm mb-6">Manage what parts of your profile are visible to the public.</p>
        ...
        <form method="post" action="{{ route('profile.toggle-visibility') }}" x-data="{ isPublic: {{ $user->is_public ? 'true' : 'false' }} }" class="mb-6">
            @csrf @method('patch')
            <div class="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-purple-500/20">
                <div>
                    ...
                    <p class="text-xs text-purple-300/60 mt-0.5">
        ...
        <form method="post" action="{{ route('profile.visibility') }}">
            ...
            <p class="text-xs font-semibold text-purple-300/70 uppercase tracking-widest mb-3">What to show publicly</p>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-5">
                ...
                <label class="flex items-center justify-between px-3 py-2.5 rounded-lg bg-white/5 border border-white/5 hover:border-purple-500/30 cursor-pointer transition-colors">
            ...
            <button type="submit"
                    class="btn-glow px-8 py-2.5 rounded-lg font-bold">
    ...
    {{-- Always private note --}}
    <p class="text-xs text-purple-300/50 flex items-center gap-1.5 px-1">
        <i class="fas fa-lock text-purple-400/30"></i>
