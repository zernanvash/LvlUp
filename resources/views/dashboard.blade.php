...
<div class="max-w-7xl mx-auto space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- Total XP -->
        <div class="glow-border rounded-2xl p-6 bg-gradient-to-br from-purple-900/40 to-purple-950/40 backdrop-blur card-hover">
            <div class="flex items-center justify-between mb-4">
            ...
            <h3 class="text-3xl font-display font-bold text-white mb-1">{{ number_format(auth()->user()->total_xp) }}</h3>
            <p class="text-sm text-purple-300">Total Experience</p>
        </div>

        <!-- Projects Count -->
        <div class="glow-border rounded-2xl p-6 bg-gradient-to-br from-blue-900/40 to-blue-950/40 backdrop-blur card-hover">
            <div class="flex items-center justify-between mb-4">
        ...
        <!-- Skills Unlocked -->
        <div class="glow-border rounded-2xl p-6 bg-gradient-to-br from-pink-900/40 to-pink-950/40 backdrop-blur card-hover">
            <div class="flex items-center justify-between mb-4">
            ...
            <h3 class="text-3xl font-display font-bold text-white mb-1">{{ auth()->user()->unlockedNodes->count() }}</h3>
            <p class="text-sm text-pink-300">Skills Mastered</p>
        </div>

        <!-- Achievement Progress -->
        <div class="glow-border rounded-2xl p-6 bg-gradient-to-br from-amber-900/40 to-amber-950/40 backdrop-blur card-hover">
            <div class="flex items-center justify-between mb-4">
    ...
    <div>
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-purple-300">Showcase your journey to mastery</p>
            </div>
            <a href="/projects/create" class="btn-glow bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 px-6 py-3 rounded-xl font-bold shadow-lg transition">
        ...
        @if($projects->isEmpty())
        <!-- Empty State -->
        <div class="glow-border rounded-3xl p-12 text-center bg-gradient-to-br from-purple-900/20 to-pink-900/20 backdrop-blur">
            <div class="w-32 h-32 mx-auto mb-6 bg-gradient-to-br from-purple-500/20 to-pink-500/20 rounded-full flex items-center justify-center">
            ...
            <a href="/projects/create" class="inline-flex items-center gap-2 btn-glow bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 px-8 py-4 rounded-xl font-bold shadow-lg transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="group relative">
                ...
                <div class="glow-border rounded-2xl overflow-hidden bg-gradient-to-br from-[#2d1b4e]/80 to-[#1a1d3e]/80 backdrop-blur card-hover rarity-{{ $rarity }} border-2 border-{{ $rarity === 'legendary' ? 'amber' : ($rarity === 'epic' ? 'purple' : ($rarity === 'rare' ? 'blue' : 'gray')) }}-500/30">
                    <!-- Project Header -->
                    <div class="relative h-48 bg-gradient-to-br from-purple-600/20 to-pink-600/20 border-b-2 border-white/10 overflow-hidden">
                        @if($project->thumbnail)
                            <img src="{{ $project->thumbnail }}" class="w-full h-full object-cover opacity-50" alt="{{ $project->name }}">
                        @else
                        ...
                        <div class="absolute top-4 right-4 flex gap-1">
                            @for($i = 0; $i < (match($rarity) { 'legendary' => 5, 'epic' => 4, 'rare' => 3, default => 2 }); $i++)
                                <i class="fas fa-star text-{{ $rarity === 'legendary' ? 'amber' : ($rarity === 'epic' ? 'purple' : ($rarity === 'rare' ? 'blue' : 'gray')) }}-400 text-sm"></i>
                            @endfor
                        </div>
                        
                        <!-- Project ID Badge -->
                        <div class="absolute bottom-4 left-4 bg-black/60 backdrop-blur px-3 py-1 rounded-lg">
                            <span class="font-mono text-xs text-purple-300">ID: {{ str_pad($project->id, 4, '0', STR_PAD_LEFT) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-3">
                            ...
                            @if($project->is_featured)
                                <span class="flex-shrink-0 ml-2 px-2 py-1 bg-amber-500/20 border border-amber-500/30 rounded text-xs text-amber-300">
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            @foreach($project->skills->take(3) as $skill)
                                <span class="px-3 py-1 bg-{{ $skill->rarity === 'legendary' ? 'amber' : ($skill->rarity === 'epic' ? 'purple' : 'blue') }}-500/20 border border-{{ $skill->rarity === 'legendary' ? 'amber' : ($skill->rarity === 'epic' ? 'purple' : 'blue') }}-500/30 rounded-lg text-xs font-bold text-{{ $skill->rarity === 'legendary' ? 'amber' : ($skill->rarity === 'epic' ? 'purple' : 'blue') }}-300">
                                    <i class="{{ $skill->icon }}"></i> {{ $skill->name }}
                                </span>
                            @endforeach
                            @if($project->skills->count() > 3)
                                <span class="px-3 py-1 bg-purple-500/10 border border-purple-500/20 rounded-lg text-xs text-purple-400">
                        ...
                        <div class="flex items-center justify-between pt-4 border-t-2 border-white/10">
                            ...
                            <div class="flex items-center gap-2 bg-purple-500/20 px-3 py-1 rounded-lg">
                                ...
                                <span class="font-display text-sm font-bold text-purple-300">+{{ $project->xp_reward }} XP</span>
                            </div>
                        </div>
                    </div>

                    <!-- Hover Action Overlay -->
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-8 gap-3">
                        <a href="/projects/{{ $project->id }}" class="btn-glow bg-purple-600 hover:bg-purple-500 px-6 py-3 rounded-xl font-bold shadow-lg transition">
                            <span class="relative z-10">View Details</span>
                        </a>
                        @if($project->github_url)
                        <a href="{{ $project->github_url }}" target="_blank" class="bg-gray-700 hover:bg-gray-600 px-6 py-3 rounded-xl font-bold shadow-lg transition">
