...
@section('page_title', 'Edit Resume')
@section('page_subtitle', 'Update your AI-tailored resume')
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="glow-border rounded-3xl bg-gradient-to-br from-[#2d1b4e]/60 via-[#1a1d3e]/80 to-[#2d1b4e]/60 backdrop-blur-xl">
        <div class="relative p-8">
            <form action="{{ route('resume.update', $resume) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <section class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="job_description" rows="6"
                            ...
                            class="w-full bg-[#0a0e27]/70 border border-purple-500/40 rounded-xl px-4 py-3 text-white placeholder-purple-400/40
                                   focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-500/30
                                   hover:border-purple-400/60 transition resize-none font-medium">{{ old('job_description', $resume->job_description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="target_keywords"
                            value="{{ old('target_keywords', $resume->target_keywords) }}"
                            ...
                            @foreach(['professional' => 'fas fa-briefcase', 'creative' => 'fas fa-palette', 'executive' => 'fas fa-crown', 'concise' => 'fas fa-bolt'] as $tone => $icon)
                                <label class="relative cursor-pointer">
                                    <input type="radio" name="tone" value="{{ $tone }}"
                                        {{ old('tone', $resume->tone ?? 'professional') === $tone ? 'checked' : '' }}
                                        class="sr-only peer">
                            ...
                            @foreach(['modern', 'classic', 'minimal', 'creative', 'executive', 'tech'] as $tpl)
                                <label class="relative cursor-pointer">
                                    <input type="radio" name="template" value="{{ $tpl }}"
                                        {{ old('template', $resume->template ?? 'modern') === $tpl ? 'checked' : '' }}
                                        class="sr-only peer">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="url" name="linked_in"
                                    value="{{ old('linked_in', $resume->linked_in) }}"
                ...
                <section class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="work_experience" rows="8"
                            placeholder="List your roles, companies, dates and key achievements…"
                            class="w-full bg-[#0a0e27]/70 border border-purple-500/40 rounded-xl px-4 py-3 text-white placeholder-purple-400/40
                                   focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-500/30
                                   hover:border-purple-400/60 transition resize-none font-mono text-sm leading-relaxed">{{ old('work_experience', $resume->work_experience) }}</textarea>
                    </div>
                </section>
                ...
                <section class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="education_details" rows="3"
                            ...
                            class="w-full bg-[#0a0e27]/70 border border-purple-500/40 rounded-xl px-4 py-3 text-white placeholder-purple-400/40
                                   focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-500/30
                                   hover:border-purple-400/60 transition resize-none font-medium">{{ old('education_details', $resume->education_details) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="certifications" rows="3"
                            ...
                            class="w-full bg-[#0a0e27]/70 border border-purple-500/40 rounded-xl px-4 py-3 text-white placeholder-purple-400/40
                                   focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-500/30
                                   hover:border-purple-400/60 transition resize-none font-medium">{{ old('certifications', $resume->certifications) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="spoken_languages"
                                value="{{ old('spoken_languages', $resume->spoken_languages) }}"
                    ...
                    <div>
                        ...
                        <textarea name="bio_seed" rows="3"
                            ...
                            class="w-full bg-[#0a0e27]/70 border border-purple-500/40 rounded-xl px-4 py-3 text-white placeholder-purple-400/40
                                   focus:outline-none focus:border-purple-400 focus:ring-2 focus:ring-purple-500/30
                                   hover:border-purple-400/60 transition resize-none font-medium">{{ old('bio_seed', $resume->bio_seed) }}</textarea>
                    </div>
                </section>
                        ...
                        @foreach($projects as $project)
                            <label class="relative flex items-start gap-3 p-4 rounded-2xl border border-purple-500/20
                                ...
                                <input type="checkbox"
                                    ...
                                    value="{{ $project->id }}"
                                    {{ in_array($project->id, old('selected_project_ids', $resume->selected_project_ids ?? [])) ? 'checked' : '' }}
                        ...
                        @foreach($skills as $skill)
                            <label class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-purple-500/30
                                ...
                                <input type="checkbox"
                                    ...
                                    value="{{ $skill->id }}"
                                    {{ in_array($skill->id, old('selected_skill_ids', $resume->selected_skill_ids ?? [])) ? 'checked' : '' }}
                                    class="sr-only">
                ...
                {{-- Regenerate notice --}}
                <div class="flex items-start gap-3 bg-amber-900/20 border border-amber-500/20 text-amber-300/70 px-4 py-3 rounded-2xl text-sm">
                    <i class="fas fa-info-circle mt-0.5 flex-shrink-0 text-amber-400/70"></i>
                    Saving updates your resume info and recalculates the keyword match score. Use
                    <strong class="text-amber-300/90 mx-1">Regenerate PDF</strong>
                    after saving to rebuild the AI content with all updated fields.
                </div>
